...
void loop2 (SDL_Surface *surface, int *yt, int *middlet, int *failt, int *possiblet, int *i2t)
{
	...
	while (i2<height-2-y && possible == 0 && middle>3 && middle <width-3)
	{
...
void Getboxes(SDL_Surface *surface, int x1, int y1, int x2, int y2, int x3, int y3, int x4, int y4)
{
	...
	while (i2 < 9)
	{
		...
		{	
			name[4]=(char)(i+48);
			name[3]=(char)(i2+48);
			int x = x4 + (x1-x4)/9*i+ (x3-x4)/9*i2;
			int xx = x4 + (x1-x4)/9*(i+1) + (x3-x4)/9*(i2);
			int xxx = x4 + (x3-x4)/9*(i2+1);
			int xxxx = x4 + (x1-x4)/9*(i+1) + (x3-x4)/9*(i2+1);
			...
			int y = y4 + (y1-y4)/9*i + (y3-y4)/9*i2;
			int yy = y4 + (y1-y4)/9*(i+1) + (y3-y4)/9*i2;
			int yyy = y4 + (y3-y4)/9*(i2+1);
			int yyyy = y4 + (y1-y4)/9*(i+1)+(y3-y4)/9*(i2+1);
			if (y>yy)
			{
				topy=yy;
			}
			...
			img=SDL_CreateRGBSurface(0,botx-topx,boty-topy,32,0,0,0,0);
...
void FindCorners(SDL_Surface *surface)
{
	...
	Getboxes(surface, x1, y1, x2, y2, x3, y3, x4, y4);
}
